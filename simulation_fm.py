...
class Namelist:
    def __init__(self,wpspath,wrfpath):
        self.wps = {
            'share':{
                'max_dom':1,
                'start_date':'2024-09-16_00:00:00',
                'end_date':['2024-09-16_06:00:00'],
            ...
            'geogrid':{
                ...
                'dx':3000, ##### fm 不用变
                'dy':3000, #####
                'ref_lat':36.345,  # denote center point
                'ref_lon':117.210,
                'truelat1':30,
                'truelat2':60,
                'stand_lon':116.916,  #normally=ref_lon
                },
            'ungrib':{
        ...
        self.wrf = {
            ...
            "domains": {
                ...
                "e_sn": [601],
                "e_vert": [38],
                "num_metgrid_levels": 34,     ######FNL 34/27 老数据是27   ERA5 38
                "dx": [3000],   ####fm不用变
                "dy": [3000],
                },
            "physics": {
    ...
    def record(self,recordpath):
        ...
        os.system(f'cp  {wpspath}/namelist.wps {recordpath}/fm{time}namelist.wps')
        os.system(f'cp {wrfpath}/namelist.input {recordpath}/fm{time}namelist.input')
        os.system(f'cp {recordpath}/../simulation_fm.py {recordpath}/{time}simulation_fm.py')
...
class wps_class:
    ...
    def ungrib(self):
        os.chdir(self.wpspath)
        os.system('rm FNL* PL* SF* met_em* PFILE* sf* pl*')
        if self.datacat=="FNL":
...
wrfpath='/public23/home/sc61341/gsj/model_1.0/test/em_real'
...
cpus= int(nml.wps['geogrid']['e_we']/25) *  int(nml.wps['geogrid']['e_sn']/25)  #according to staff suggenstion and the best way to cut the domain into many tiles which
# requires the number of grids in both x,y direction to be 25*n form.
